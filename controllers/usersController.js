...
exports.updateUser = async (req, res) => {
  try {
    ...
    if (invalidFields.length) {
      return res.status(400).json({ message: `Le ou les champs qui n'existent pas : ${invalidFields.join(', ')}` });
    }
    ...
    const updatedProperties = _.omitBy(newUser.dataValues, (value, key) => _.isEqual(value, oldUser.dataValues[key]));
